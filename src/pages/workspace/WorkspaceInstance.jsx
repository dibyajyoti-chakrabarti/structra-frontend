...
const WorkspaceInstance = () => {
  ...
  const { workspaceId } = useParams(); // Grabs the ID from the URL (/app/ws/:workspaceId)
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 gap-3">
              {systems.map((system) => (
                <div 
                  key={system.id} 
                  className="bg-white p-4 rounded-xl border border-gray-200 flex justify-between items-center hover:border-blue-400 hover:shadow-md transition-all cursor-pointer group"
                >
                  <div>
                    <h3 className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{system.name}</h3>
                    <p className="text-sm text-gray-500">Last updated {system.updated}</p>
                  </div>
                  <span className={`px-3 py-1 rounded-lg text-xs font-bold ${
                    system.status === 'Active' ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-500'
                  }`}>
                    {system.status}
                  </span>
                </div>
