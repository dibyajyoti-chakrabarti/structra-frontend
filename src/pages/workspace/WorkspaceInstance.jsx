  ...
  return (
    <div className="pb-10">
       <div className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Workspace {workspaceId}</h1>
            <p className="text-gray-500 text-sm md:text-base">Overview of your systems and evaluations.</p>
          </div>
          <button 
       ...
       <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-10">
          {stats.map((stat, i) => (
            <div key={i} className="bg-white p-5 md:p-6 rounded-2xl border border-gray-200 shadow-sm">
       ...
       <div className="grid grid-cols-1 gap-3">
          {systems.map((system) => (
            <div key={system.id} className="bg-white p-4 rounded-xl border border-gray-200 flex justify-between items-center hover:border-blue-400 transition-all cursor-pointer">
              <div>
                <h3 className="font-bold text-gray-900 text-sm md:text-base">{system.name}</h3>
                <p className="text-xs md:text-sm text-gray-500">Last updated {system.updated}</p>
              </div>
              <span className={`px-2 py-1 md:px-3 md:py-1 rounded-lg text-[10px] md:text-xs font-bold ${system.status === 'Active' ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-500'}`}>
                {system.status}
              </span>
            </div>
          ))}
